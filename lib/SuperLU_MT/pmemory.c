...
/*/
#define XPAND_HINT(memtype, new_next, jcol, param) {\
TRACE("Storage for %12s exceeded; Current column %d; Need at least %d;\n",\
	memtype, jcol, new_next); \
TRACE("You may set it by the %d-th parameter in routine sp_ienv().\n", param); \
SUPERLU_ABORT("Memory allocation failed"); \
}
//*/
...
int *intMalloc(int n)
{
	...
	TRACE("SUPERLU_MALLOC failed for buf in intMalloc()\n");
	exit (1);
    }
    return (buf);
}
...
int *intCalloc(int n)
{
	...
	TRACE("SUPERLU_MALLOC failed for buf in intCalloc()\n");
	exit (1);
    }
