...
/* -------------------
   Internal prototypes
   ------------------- */
void    *pdgstrf_expand (int *, MemType,int, int, GlobalLU_t *);
void    copy_mem_double (int, void *, void *);
void    pdgstrf_StackCompress(GlobalLU_t *);
void    pdgstrf_SetupSpace (void *, int);
 ...
 */
void pdgstrf_SetupSpace(void *work, int lwork)
{
    if ( lwork == 0 ) {
...
float pdgstrf_memory_use(const int nzlmax, const int nzumax, const int nzlumax)
{
...
pdgstrf_WorkInit(int n, int panel_size, int **iworkptr, double **dworkptr)
{
	...
	*iworkptr = (int *) duser_malloc(isize, TAIL);
	...
	TRACE("pdgstrf_WorkInit: malloc fails for local iworkptr[]\n");
	return (isize + n);
    }
...
#pragma omp critical ( STACK_LOCK )
	...
	TRACE("malloc fails for local dworkptr[].");
		 ...
		 )
{
	...
	int    nzlumax = Glu->nzlumax;
    	...
    	TRACE("Can't expand MemType %d: jcol %d\n", mem_type, jcol);
...
double *doubleMalloc(int n)
{
	...
	TRACE("SUPERLU_MALLOC failed for buf in doubleMalloc()");
	exit (1);
    }
    return (buf);
}
...
double *doubleCalloc(int n)
{
	...
	TRACE("SUPERLU_MALLOC failed for buf in doubleCalloc()");
	exit (1);
    }
