...
typedef itk::Vector<float, ImageDimension> InputPixelType;
typedef itk::Vector<float, ImageDimension> OutputPixelType;
...
typedef itk::VectorResampleImageFilter<InputImageType, InputImageType> ResampleImageFilterType;
typedef double CoordRepType;
typedef itk::VectorLinearInterpolateImageFunction<InputImageType, CoordRepType> InterpolatorType;
...
{
	printf("==========================================================================\n");
	printf("ResampleDeformationField (GLISTR)\n");
#ifdef SW_VER
	printf("  Version %s\n", SW_VER);
#endif
#ifdef SW_REV
...
void usage()
{
	...
	printf("\n\n");
	printf("Example:\n\n");
	printf("ResampleDeformationField -i [input_deformation_file] -o [output_deformation_file] -r [reference_image_file] -x [ratio_x] -y [ratio_y] -z [ratio_z]\n");
}
...
int main(int argc, char* argv[])
{
    ...
    // adjust image geometry of tumor deformation field written by ForwardSolverDiffusion
    //
    // Note that the tumor deformation field has origin (0,0,0) which, however,
    // corresponds to the origin of the upsampled atlas space shifted by half a voxel
    // size, not the origin of the downsampled label map which was the actual input to
    // the ForwardSolverDiffusion program. The shift of the origin is due to the different
    // sampling grid which is one voxel wider than the input label map, i.e., 65x65x65
    // if the downsampled input label map to ForwardSolverDiffusion was of size 64x64x64.
    for (unsigned int i = 0; i < ImageDimension; i++) {
        input_origin[i] = output_origin[i] - input_spacing[i] / 2;
    }
