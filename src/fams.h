...
public:
	...
	inline void push_back(fams_pointp in_el) { vec_[nel_++] = in_el; };
...
#define D_Bs  (FAMS_BLOCKSIZE  / sizeof(fams_hash_entry))
#define D_Bs2 (FAMS_BLOCKSIZE2 / sizeof(fams_hash_entry2))

typedef fams_hash_entry block[D_Bs];
typedef fams_hash_entry2 block2[D_Bs2];
...
public:
	...
	int RunFAMS(int K, int L, int k, double percent, int jump, float width, char* pilot_fn, int prune_window = FAMS_PRUNE_WINDOW, int prune_minn = FAMS_PRUNE_MINN);
	...
	void AddDataToHash(block HT[], int hs[], fams_point& pt, int where, int Bs, int M, int which, int which2, int hjump);
	void ComputePilot(block *HT, int *hs, fams_partition *cuts, char* pilot_fn);
	void GetNearestNeighbours(fams_point& who, block *HT, int *hs, fams_partition *cuts, fams_res_cont& res, int print, int num_l[]);
	void AddDataToRes(block HT[], int hs[], fams_res_cont& res, int where, int Bs, int M, int which, unsigned short nnres, int which2, int hjump);
	unsigned short* FindInHash(block2* HT2, int *hs2, int where, int which, int M2, int hjump);
	void InsertIntoHash(block2* HT2, int *hs2, int where, int which, unsigned short **solution, int M, int hjump);
	unsigned short* GetNearestNeighbours2H(unsigned short *who, block*HT, int *hs, fams_partition* cuts, fams_res_cont& res, unsigned short **solution, block2 *HT2, int *hs2);
	void DoFAMS(block *HT,int *hs, fams_partition *cuts, block2* HT2, int *hs2);
	unsigned int DoMeanShiftAdaptiveIteration(fams_res_cont& res, unsigned short *old, unsigned short *ret);
	...
	// a boolean function which computes the distance if it is less than dist into dist_res. 
