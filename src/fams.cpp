...
{
	timeend = clock();
	time_t hours = 0, minutes = 0, seconds = 0, milliseconds = 0;
	seconds = (timeend - timestart) / CLOCKS_PER_SEC;
	milliseconds = ((1000 * (timeend - timestart)) / CLOCKS_PER_SEC) - 1000 * seconds;
	minutes = seconds/60;
	if (minutes == 0) {
...
void FAMS::AddDataToHash(block HT[], int hs[], fams_point& pt, int where, int Bs, int M, int which, int which2, int hjump)
{
	int nw = 0;
...
// computation
void FAMS::InsertIntoHash(block2* HT2, int *hs2, int where, int which, unsigned short **solution, int M, int hjump)
{
	int nw = 0;
	for (; ; where = (where + hjump) % M)
	{
...
void FAMS::GetNearestNeighbours(fams_point& who, block *HT, int *hs, fams_partition *cuts, fams_res_cont& res, int print, int num_l[])
{
	int i;
	for (i = 0; i < L_; i++) {
	...
	for (i = 0; i < L_; i++) {
		...
		AddDataToRes(HT, hs, res, m, D_Bs, M_, i, nnres1_, m2, hjump);
		num_l[i] = res.nel_;
	}
}

// perform an LSH query using in addition the second hash table
unsigned short* FAMS::GetNearestNeighbours2H(unsigned short *who, block*HT, int *hs, fams_partition* cuts, fams_res_cont& res, unsigned short **solution, block2 *HT2, int *hs2)
{
	int i;
	for (i = 0; i < L_; i++)
	{
	...
	nnres2_++;
	for (i = 0; i < L_; i++) {
		AddDataToRes(HT, hs, res, t_m_[i], D_Bs, M_, i, nnres2_, t_m2_[i], t_hjump_[i]);
	}
	return NULL;
}
...
unsigned int FAMS::DoMeanShiftAdaptiveIteration(fams_res_cont& res, unsigned short *old, unsigned short *ret)
{
...
void FAMS::AddDataToRes(block HT[], int hs[], fams_res_cont& res, int where, int Bs, int M, int which, unsigned short nnres, int which2, int hjump)
{
	int uu;
	for (; ; where = (where+hjump)%M)
	{
...
void FAMS::DoFAMS(block *HT,int *hs, fams_partition *cuts, block2* HT2, int *hs2)
{
	int jj;
	fams_res_cont res(n_);
	...
	for (jj = 0; jj < nsel_; jj++)
	{
		...
		*crtH = currentpt->window_;
		...
		for (iter = 0; NotEq(oldMean, crtMean) && (iter < FAMS_MAXITER); iter++)
		{
			if (!noLSH_)
			{
				if ((sol = GetNearestNeighbours2H(crtMean, HT, hs, cuts, res, &tMode[jj], HT2, hs2)))
				{
...
int FAMS::PruneModes(int hprune, int npmin)
{
	...
	for (cm = 0; cm < nsel_; cm++)
	{
		...
		cminDist = d_ * 1e7;
		iminDist = -1;
		for (cref = 0; cref < npm_; cref++)
		{
			cdist = 0;
			for (cd = 0; cd < d_; cd++) {
				cdist += fabs((float)prunedmodes_[cref*d_ + cd] - pmodes[cd]);
			}
			if (cdist < cminDist) {
				cminDist = cdist;
				iminDist = cref;
		...
		for (cd = 0; cd < d_; cd++) {
			pmodes[cd] = prunedmodes_[iminDist*d_ + cd];
		}
	}
	//
...
double FAMS::DoFindKLIteration(int K, int L, float* scores)
{
	...
	{ 
		for (i = 0; i < L_; i++)
		{
			...
			AddDataToHash(HT, hs, points_[j], m, D_Bs, M_, i, m2, hjump);
		}
	}
...
int FAMS::RunFAMS(int K, int L, int k, double percent, int jump, float width, char* pilot_fn, int prune_window, int prune_minn)
{
	...
	bgLog("SelectMsPoints\n");
	SelectMsPoints(percent, jump);

	// Allocate memory for the hash table
	M_ = GetPrime(3*n_*L_/(D_Bs));
	M2_ = GetPrime((int)(nsel_*20*3/D_Bs2));
	bgLog("M_ = %d, M2_ = %d\n", M_, M2_);
	if (M_ < 5 || M2_ < 5) {
		bgLog("M_ < 5 || M2_ < 5\n");
		return 1;
	...
	// Insert data into partitions
	bgLog("Insert data into partitions\n");
	for (j = 0; j < n_; j++)
	{ 
		for (i = 0; i < L_; i++)
		{
	...
	// join modes
	PruneModes(prune_window, prune_minn);
